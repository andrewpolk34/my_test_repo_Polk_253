...
jobs_log_model_3 <- train(
    as.factor(fraudulent) ~  description_numchar + comp_prof_numchar + domestic + employment_type_new + has_company_logo + n_miss_all + industry + required_education + required_experience + has_questions,
    data = jobs_train,
    method = "glm",
    family = "binomial",
    trControl = trainControl(method = "cv", number = 5, sampling = "down"),
...
```{r, eval=FALSE, echo=FALSE}
confusionMatrix(data = as.factor(ifelse(predict(jobs_log_model_3$finalModel, 
                               type = "response") > 0.5, "1", "0")) ,
                reference = as.factor(jobs_train$fraudulent),
                positive = "1")
length(as.factor(ifelse(predict(jobs_log_model_3$finalModel, 
                               type = "response") > 0.1, "1", "0")))
...
```{r, eval=FALSE, echo=FALSE}
confusionMatrix(data = as.factor(predict(jobs_log_model_3, 
                               type = "prob")),
                reference = as.factor(jobs_train$fraudulent),
                positive = "1")
```
...
```{r, eval=FALSE, echo=FALSE}
confusionMatrix(data = as.factor(ifelse(predict(jobs_lasso$finalModel,
                               type = "response") > 0.5, "1", "0")) ,
                reference = as.factor(jobs_train$fraudulent),
                positive = "1")
```
...
| Model | Description | CV Accuracy |
|-------|-------------|--------------|
|jobs_log_model_1 |three binary variables| 0.8105623 |
|jobs_log_model_2| three categorical variables| 0.533955 |
|jobs_log_model_3| all relevant variables| 0.7502988 |
|jobs_lasso| lambda = 0.007564633 | 0.7964006 |
|**jobs_tree** | cp = 0 | **0.8418798**
|**jobs_randf**| mtry = 10 | **0.8781726** |

**From our initial models, it seems the classification tree and random forest models yield the highest cross-validated accuracies**
...
| Model | Description | CV Accuracy |
|-------|-------------|--------------|
|jobs_tree_test | cp = 0 | 0.8418798 |
|**jobs_randf_test**| mtry = 10 | **0.8828125** |
